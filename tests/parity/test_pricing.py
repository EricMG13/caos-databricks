...
`pricing.priced_request` and `pricing.worst_case` over a grid of request byte
sizes and three dated `ModelPrice`s, plus the suite's invalid prices.
"""
...
def test_pricing_case_matches_the_legacy_golden(name: str) -> None:
...
def test_pricing_manifest_names_exactly_the_registered_cases() -> None:
