...
| Lakebase grants and the migration role (CF-010, OD-5) | Mints a credential for the Docker Postgres; `tests/test_platform_boot.py` boots on the bundle's grant alone (CONNECT and CREATE, DL-1) and refuses a store schema another role owns (F364). Unproven: whether the workspace provisions the app's role, and what it injects for a `postgres` resource (D49). A migration role the runtime cannot assume is the enterprise DBA's (N16). | E6 `store`, E8 |
