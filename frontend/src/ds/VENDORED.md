...
| `ActionReason.tsx`  | `components/shared/ActionReason.tsx` | `85ad0516` | `"use client"` dropped; `data-*` passes through unchanged; `reasonTitle` carries the pointer's fuller detail (the code) while the reason stays the plain sentence                                                    |
