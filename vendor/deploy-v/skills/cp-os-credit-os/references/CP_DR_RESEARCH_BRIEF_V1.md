...
- **TDR.2**, tag `<!-- table-id: cpdr.evidence -->`: `evidence_id`, `question_id`, `claim`, `claim_type`, `source`, `source_locator`, `source_date`, `source_type`, `independence_family`, `entity_period_unit_perimeter`. Use ISO source dates and actual file/URL locators. `claim_type` is fact / source_characterisation / inference / analyst_judgment, or gap on a gap row; `source_type` is primary / independent_secondary / attributed_view / gap. A gap row explicitly states what was unavailable, where/how it was sought and its limitation; it never supplies factual support. Its shape: `source_type` and `claim_type` are both `gap`; `claim` states what was unavailable and its limitation; `source` and `source_locator` name where and how it was sought; `source_date` and `independence_family` are `—`, since a search that found nothing has neither; `entity_period_unit_perimeter` names the perimeter searched, or is `—`. A `—` is valid only in those three cells of a gap row.
- **TDR.3**, tag `<!-- table-id: cpdr.findings -->`: `question_id`, `answer`, `evidence_ids`, `contrary_evidence`, `resolution_status`, `uncertainty`, `proposed_consequence`. Evidence IDs are semicolon-separated. Record exactly one ANSWERED or UNRESOLVED row per question. An UNRESOLVED finding cites its own question's gap row, and any partial evidence, in `evidence_ids`; it never needs an invented source. Contrary evidence needs locators or an explicit account of the disconfirming search and absence of a finding; empty text is invalid.
...
Coverage = ANSWERED questions / all locked questions × 100, rounded half up. Report Complete only at full coverage; otherwise Complete with Gaps. Never remove difficult questions after searching. UNRESOLVED questions block their named consumers while independent work continues. Evidence missing for a question never blocks the run: record that question UNRESOLVED with its gap row, and the run is Complete with Gaps. Record a genuinely blocked research run (missing research capability, access, approval or identity) as Blocked; it remains unaccepted as downstream evidence.
