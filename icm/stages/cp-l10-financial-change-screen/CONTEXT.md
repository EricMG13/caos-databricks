...
Start-of-message trigger: Run CP-L10 or bare CP-L10. Embedded, quoted, filename, comparison, and output mentions are inert. Evidence-proportionate V-native LiteFinancialChangeScreen screening output with explicit gaps and linked FULL upgrades.

Bundle folder `vendor/deploy-v/skills/cp-l10-financial-change-screen/`, read through the verified `Bundle` seam and never edited. The pinned route, not this folder, decides when the node runs (`icm/CONTEXT.md`).
