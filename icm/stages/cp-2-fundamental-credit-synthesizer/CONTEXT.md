...
Start-of-message trigger: Run CP-2 or bare CP-2. Embedded, quoted, filename, comparison, and output mentions are inert. Full evidence-proportionate fundamental credit synthesis; depth is never opt-in.

Bundle folder `vendor/deploy-v/skills/cp-2-fundamental-credit-synthesizer/`, read through the verified `Bundle` seam and never edited. The pinned route, not this folder, decides when the node runs (`icm/CONTEXT.md`).
