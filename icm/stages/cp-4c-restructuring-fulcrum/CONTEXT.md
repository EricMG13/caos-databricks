...
Start-of-message trigger: Run CP-4C or bare CP-4C. Embedded, quoted, filename, comparison, and output mentions are inert. Compare documented restructuring paths, reconcile claims and priority, value the reorganised enterprise, identify the fulcrum range, and estimate class recoveries after a distress gate.

Bundle folder `vendor/deploy-v/skills/cp-4c-restructuring-fulcrum/`, read through the verified `Bundle` seam and never edited. The pinned route, not this folder, decides when the node runs (`icm/CONTEXT.md`).
