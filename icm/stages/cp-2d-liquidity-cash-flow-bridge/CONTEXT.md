...
Start-of-message trigger: Run CP-2D or bare CP-2D. Embedded, quoted, filename, comparison, and output mentions are inert. Build a source-grounded near-term liquidity bridge, cash runway and months to empty. Trigger on cash burn, working capital, revolver access and mandatory cash uses.

Bundle folder `vendor/deploy-v/skills/cp-2d-liquidity-cash-flow-bridge/`, read through the verified `Bundle` seam and never edited. The pinned route, not this folder, decides when the node runs (`icm/CONTEXT.md`).
