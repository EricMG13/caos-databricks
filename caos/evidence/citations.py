...
def verify_citations(
    ...
    *,
    ...
    exact blocks the node was handed. Under `ANY_RUN` the match must lie
    wholly within delivered lines: a quote on an undelivered page, or wrapping
    onto an undelivered line, refuses `CITATION_NOT_DELIVERED`. Under
    `WHOLE_LINE` a match is exactly one block, and delivery is judged against
    that one block (R24-16): the prompt calls a shown block whole citable
    evidence, and a page map may show a source line's first block while
    withholding its continuation, so requiring the *line's* every block would
    refuse a quote of exactly what was shown. Ambiguity is still counted over
    the whole page (`docs/DECISIONS.md` section 44.5), so a quote repeated on
    a line the node never saw is ambiguous rather than resolved to the copy it
    did -- over every run of the page under `ANY_RUN`, over every line of it
    under `WHOLE_LINE`.
